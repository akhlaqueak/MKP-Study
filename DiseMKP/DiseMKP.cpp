 ...
 */
 ...
 #include <unistd.h>
 #ifdef _WIN32
 // Windows does not have <sys/resource.h>, so we provide a stub for getrusage
 #include <windows.h>
 ...
 inline int getrusage(int, rusage* usage) {
	 ...
	 if (GetProcessTimes(GetCurrentProcess(), &createTime, &exitTime, &kernelTime, &userTime)) {
		 ...
		 usage->ru_utime.tv_sec = (long)(uli.QuadPart / 10000000ULL);
		 usage->ru_utime.tv_usec = (long)((uli.QuadPart % 10000000ULL) / 10);
		 return 0;
	 }
   ...
   printf("compiled at %s,%s\n",__TIME__,__DATE__);
	 ...
	 int main(int argc, char *argv[]) {
	   ...
	   if (build_simple_graph_instance(argv[1])) {
		 ...
		 printf(
		  ">>%s @ %s |V| %d |E| %d K %d MaxKPX %d Total %4.2lf\n",
		  argv[1],getInstanceName(argv[1]),NB_NODE_O, NB_EDGE_O, PARA.KX,
		  MAX_KPX_SIZE, SEARCH_TIME);	  

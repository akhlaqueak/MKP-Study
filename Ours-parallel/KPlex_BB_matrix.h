...
public:
	...
	void kPlex(ui K_, ui UB_, std::vector<ui> &kplex, bool must_include_0)
	{
		...
		if (R_end && best_solution_size.load() < _UB_)
			BB_search(0, R_end, 1, must_include_0, true, TIME_NOW);
	}
...
private:
	...
	void BB_search(ui S_end, ui R_end, ui level, bool choose_zero, bool root_level = true, std::chrono::steady_clock::time_point st = TIME_NOW)
	{
...
#pragma omp task firstprivate(ctx1, u, S_end, R_end, level)
	...
	void collect_removable_vertices_based_on_total_edges(ui S2_n, ui S_end, ui R_end, ui level)
	{
		...
		for (ui i = 0; i < vp.size(); i++)
		{
			...
			while (true)
			{
				if (j == new_n)
					j = i + 1;
				if (j >= vp.size() || ub > best_sz || ub + vp.size() - j <= best_sz)
					break;
	...
	void restore_SR_and_edges(ui &S_end, ui &R_end, ui old_S_end, ui old_R_end, ui level, ui old_removed_edges_n)
	{
		...
		{ // insert u back into R
			ui u = SR[R_end];
			// if(SR_rid[u] != R_end or level_id[u] != level)for(ui i=R_end;i<old_R_end; i++) cout<<SR[i]<<"->"<<SR_rid[SR[i]]<<" ";
			// cout<<endl;
			assert(level_id[u] == level && SR_rid[u] == R_end);
			level_id[u] = n;
